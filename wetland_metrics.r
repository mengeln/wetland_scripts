...
Wetland_IBI <- function(data, DistinctCode=F){
  ...
  for(i in 1:20){
    ...
    ###Percent Coleoptera###
    metrics[[i+80]] <- ddply(data, "SampleID",
                             function(d){
                               100*sum(d[which(d$Order == "Coleoptera"), datalength + i], na.rm=T)/sum(d[, datalength + i], na.rm=T)
                             })[, 2]
    
    ###Percent EOT###
    metrics[[i+100]] <- ddply(data, "SampleID",
                              function(d){
                                100*sum(d[d$Order %in% c("Ephemeroptera", "Odonata", "Trichoptera"), datalength + i], na.rm=T)/sum(d[, datalength + i], na.rm=T)
                              })[, 2]

    ###Percent Tanypodinae/Chironomidae###
    metrics[[i+120]] <- ddply(data, "SampleID",
                              function(d){
                                ifelse(length(which(d$Family == "Chironomidae"))>0,
                                       100*sum(d[d$subfamily == "Tanypodinae", datalength + i], na.rm=T)/sum(d[d$Family == "Chironomidae", datalength + i], na.rm=T),
                                0)
                              })[, 2]
    
    ###Percent Dominant###
    metrics[[i+140]] <- ddply(data, "SampleID",
                              function(d){
                                100*sum(d[order(d[, datalength + i], decreasing=T), datalength + i][1:3])/sum(d[, datalength + i], na.rm=T)
                              })[, 2]
  }
  write.csv(data, file="iterations.csv")
  ...
  ###Extra stuff (TEMP)
  results$Tanypodinae <- ddply(data, "SampleID",
                               function(d){
                                 sum(d$Result[d$subfamily == "Tanypodinae"], na.rm=T)
                               })[, 2]
  results$Chironominae <- ddply(data, "SampleID",
                               function(d){
                                 sum(d$Result[d$subfamily == "Chironominae"], na.rm=T)
                               })[, 2]
  results$Chironomidae <- ddply(data, "SampleID",
                                function(d){
                                  sum(d$Result[d$Family == "Chironomidae"], na.rm=T)
                                })[, 2]
  ###Return results###
  results
}
...
write.csv(results, file="Wetlands_IBI_metrics_v2.csv")
